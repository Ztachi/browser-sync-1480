 ...
 * @Last Modified time: 2018-01-04 11:15:01
 */
...
//编译sass
function f_sass() {
    return gulp.src(paths.app + '/**/*.{scss,css}')
        .pipe(plumber())
        .pipe(sass({
                precision: 2,
...
gulp.task('default', () => {
    ...
    sequence(['clean:dist', 'clean:static', 'clean:cache'],'htmlMin', 'jsmin', 'copy:common', 'sass', 'svgsprite', 'imgMin');
    //sass文件监听
    watch(paths.app + '/**/*.{scss,css}', f_sass);
    //js文件监听
    watch(paths.js + '/**/*.js', f_js);
    //common js文件监听
    watch(paths.common + '/**/*.js', f_common);
    //img文件监听
    watch(paths.img + '/**/*.{png,jpg,gif,ico,svg}', f_imgMin);
    //svg文件监听
    watch(paths.svg + '/**/*.svg', f_svgsprite);
    //html文件监听
    watch(paths.app + '/**/*.html', f_htmlMin);
